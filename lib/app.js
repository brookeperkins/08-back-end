...
app.get('/queens', async(req, res) => {
  ...
  SELECT q.id, name, image_url, quote, w.winner_type 
  FROM queens as q
  JOIN winners AS w
  ON q.winner_id = w.id
  `);
...
app.post('/queens', async(req, res) => {
  ...
  VALUES($1, $2, $3, $4, $5)
  RETURNING *
`, [aQueenIsBorn.name, aQueenIsBorn.image_url, aQueenIsBorn.quote, fakeUser.id, aQueenIsBorn.winner_id]);
...
app.put('/queens/:id', async(req, res) => {
  ...
  try {
    ...
    const data = await client.query(`
      UPDATE queens
        SET name=$1, image_url=$2, quote=$3, winner_id=$4
        WHERE queens.id = $5
        RETURNING *
    `, [thatNewQueen.name, thatNewQueen.image_url, thatNewQueen.winner_id, thatNewQueen.quote, queenId]);
